...
- (Song *) songFromSLTitle:(SLTitle *)title;
- (NSIndexSet *) trackIdsFromArray:(NSArray *)array;
...
- (void) fetchedCurrentPlaylist:(NSArray *)playlist {
	...
	for (int i = 0; i < [playlist count]; i++)
		[result addObject:[self songFromSLTitle:[playlist objectAtIndex:i]]];
	
	[_currentPlayList release];
	_currentPlayList = [result retain];
...
- (void) fetchedCurrentPlaylistTrack:(SLTitle *)aTitle forIndex:(int)index {
	[[NSNotificationCenter defaultCenter] postNotificationOnMainThreadWithName:nMusicServerClientFetchedTitleForPlaylist
																		object:self
																	  userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[self songFromSLTitle:aTitle], dSong, [NSNumber numberWithInt:index], dSongPosition, nil]];
}
...
- (void) fetchedCompleteDatabase:(NSArray *)titles {
	...
	for (int i = 0; i < [titles count]; i++)
		[songs addObject:[self songFromSLTitle:[titles objectAtIndex:i]]];
...
- (void) fetchedCurrentSong:(SLTitle *)song {
	[[NSNotificationCenter defaultCenter] postNotificationOnMainThreadWithName:nMusicServerClientCurrentSongChanged
																		object:self
																	  userInfo:[NSDictionary dictionaryWithObject:[self songFromSLTitle:song] forKey:dSong]];
...
- (Song *) songFromSLTitle:(SLTitle *)title {
	...
	[song setTitle:[title title]];
	
	int titleId = [title titleId];
	[song setUniqueIdentifier:[NSData dataWithBytes:&titleId length:sizeof(titleId)]];
	
	if ([title artist])
		[song setArtist:[[title artist] title]];
	if ([title album])
		[song setAlbum:[[title album] title]];
	if ([title trackNumber] >= 0)
		[song setTrack:[NSString stringWithFormat:@"%d", [title trackNumber]]];
	if ([title genre])
		[song setGenre:[title genre]];
	if ([title duration] >= 0)
		[song setTime:[title duration]];
...
- (NSIndexSet *) trackIdsFromArray:(NSArray *)array {
	NSMutableIndexSet *trackIds = [NSMutableIndexSet indexSet];
	int trackId;
